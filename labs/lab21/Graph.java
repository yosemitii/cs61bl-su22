...
public class Graph implements Iterable<Integer> {
    ...
    public int inDegree(int v) {
        // TODO: YOUR CODE HERE
        int count = 0;
        for (LinkedList<Edge> llst: adjLists) {
            if (llst.isEmpty()) {
                break;
            }
            for (Edge edges: llst) {
    ...
    public boolean pathExists(int start, int stop) {
        // TODO: YOUR CODE HERE
        DFSIterator dfsIterator = new DFSIterator(start);
        while(dfsIterator.hasNext()) {
            if (dfsIterator.next() == stop) {
                return true;
            }
            else {
